 ...
 * 기획서 7장의 "AI 봇 베팅 판단"은 decideBotAction과 동일한 입출력으로
 * Claude API 호출부를 이 파일에 붙이면 된다 (TODO 표시 참고).
 */
...
export interface BotDecision {
  ...
  /** raise 시 추가로 올리는 금액 */
  amount?: number;
  /** 결정 이유 (게임 로그 연출용 — 추후 Claude API가 생성) */
  reason: string;
}
 ...
 * 봇의 베팅 결정.
 *
 * TODO(AI 활용): 이 함수를 async로 바꾸고, 아래 휴리스틱 대신 Claude API에
 * BotDecisionInput(판 상태)과 페르소나 프롬프트를 전달해 BotDecision(JSON)을
 * 받아오도록 교체 — API 실패 시 현재 휴리스틱을 폴백으로 유지.
 */
export function decideBotAction(input: BotDecisionInput, rng: () => number = Math.random): BotDecision {
  ...
  strength += (rng() - 0.5) * 0.2;
  const bluffing = rng() < (persona === 'aggressive' ? 0.15 : 0.07);
