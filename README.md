...
| 로열의 예언 | 골드 | 셔플 | 셔플 시 낮은 확률로 유리한 카드가 들어올 확률 소폭 상승(연출용) | 싱글/멀티 |
...
**AI 봇**
- 매 액션마다 몬테카를로로 자기 승률(에퀴티)을 계산해 팟오즈와 비교하는 방식으로 베팅 결정 —
  콜에 필요한 승률보다 실제 승률이 낮으면 접고, 충분히 높으면 레이즈한다
- 공격적/신중한 페르소나는 그 판단 임계값의 오프셋으로 표현 — 여기에 소폭의 랜덤 흔들림과
  저빈도 블러핑(4~6%)이 얹혀 매번 같은 상황에서 같은 선택만 하지는 않는다
- 플레이어에게 보이는 대사와 판단 근거 수치를 분리해서 내보낸다 — 말풍선·게임 로그에는 대사만
  나가고(`🤖 봇: 레이즈 (+150) — "좋은 패다. 레이즈."`), 수치는 `BotDecision.detail`에만 담겨
  셀프플레이 튜닝·디버깅에서만 쓰인다
- 임계값은 감이 아니라 셀프플레이 측정으로 정했다 — 아래 "셀프플레이 기반 봇 튜닝" 참고
- 실제 LLM 호출이 아님 — `decideBotAction` 인터페이스는 Claude API로 교체하기 쉽게 설계된 상태(TODO, 미구현)

**몬테카를로 에퀴티 엔진**

**기성 홀덤 확률표를 쓸 수 없어 직접 구현했다.** 이 게임은 조커(황금 뒤집개), 홀카드의 숫자·무늬
변경(카멜레온), 카드 교체·맞교환(카드 재구성/당근이세요?) 같은 증강이 판 자체를 바꾼다. "AA는
헤즈업 85%" 같은 사전 계산 표는 조커 한 장만 끼어도 맞지 않는다. 그래서 매 판의 실제 카드
상태에서 직접 롤아웃을 돌려 승률을 구한다.

- 남은 덱에서 상대 홀카드와 미공개 커뮤니티 카드를 무작위로 채워 끝까지 돌린 뒤,
  `handEvaluator`로 승/무/패를 센다. 무승부는 인원수로 나눈 지분으로 더한다
- 같은 계산기(`engine/equity.ts`)를 **봇 판단(롤아웃 1200회)과 화면의 실시간 승률 표시(800회)가
  공유**한다 — 봇이 보는 승률과 플레이어가 보는 승률이 같은 코드에서 나온다
- 조커가 섞이면 `handEvaluator`가 무늬 배정을 전부 전개하느라 비용이 급증해서, 조커가 있으면
  롤아웃 횟수를 자동으로 절반으로 줄인다 (보드에 조커 + 상대 4명 기준 276ms → 137ms)

정확도 검증 (`server/src/engine/equity.test.ts`):

| 상황 | 계산값 | 기대값 |
|---|---|---|
| AA vs 랜덤 1명 (프리플랍) | 0.856 | ≈ 0.85 |
| AA vs 랜덤 3명 (프리플랍) | 0.644 | 인원이 늘면 하락해야 함 |
| 리버 확정 승리 (쿼드 에이스) | 1.000 | 1.0 |
| 리버 확정 무승부 (보드 플레이) | 0.500 | 0.5 |

판단 1회 소요 시간은 프리플랍 헤즈업 36ms, 프리플랍 4인 88ms.

**셀프플레이 기반 봇 튜닝**

봇이 약한 패로 콜·레이즈를 과하게 한다는 관찰에서 출발했다. 몇 판 눈으로 보고 감으로 고치는
대신, `server/scripts/selfplay.ts`로 봇끼리 2000판씩 자동 대전시켜 측정했다. Colyseus를 띄우지
않고 엔진만으로 블라인드 → 4스트리트 베팅 → 쇼다운을 직접 돌리는 하네스라 판당 왕복·연출
딜레이가 없다. 증강은 아직 넣지 않았고, 나중에 붙일 수 있게 훅 자리만 비워뒀다.

**먼저 지표 자체가 믿을 만한지부터 검증했다.** 두 페르소나를 완전히 동일한 설정으로 맞춘
대조군을 돌렸다 — 전략이 같으니 칩 수지는 0이 나와야 한다. 그런데 2000판 기준 −73.6, −17.2가
나왔다. 칩 수지(BB/100)는 노이즈 바닥이 ±50~75라 설정 간 실제 차이가 그 폭에 묻힌다는 뜻이고,
그대로 썼다면 노이즈를 개선으로 착각할 뻔했다.

그래서 판정 지표를 **'손해 보는 콜 비율'**(실제 에퀴티 < 팟오즈인데도 콜한 비율)로 바꿨다.
핸드 결과가 아니라 판단 8000건을 평균 내는 값이라 회차 간 0.6%p 안에서 재현된다.
...
바꾼 값은 `BOT_EQUITY_CONFIG`의 `personaOffset.aggressive`(0.06 → 0.02)와
`raiseFacingBet`(0.68 → 0.72) 두 개뿐이다. 임계값 전부를 인자로 덮어쓸 수 있어
(`npm run selfplay -- --raiseFacingBet=0.72`) 같은 판수로 설정을 바꿔가며 비교할 수 있다.

**UI/연출**
- 실시간 족보 하이라이트 (`[A,K,Q,J,10] 로열 스트레이트 플러시` 형태 캡슐, 플랍 이후 매 액션마다 갱신) — 싱글/멀티 공통
- 실시간 승률 표시 (`승률 63.2%`, 족보 캡슐 옆) — 싱글/멀티 공통. 카드가 열리거나 상대가 죽을
  때만 다시 계산한다. **본인 홀카드와 공개된 커뮤니티 카드로만 계산** — 상대 홀카드는 애초에
  클라이언트에 오지 않고, 상대는 "몇 명 남았는가"라는 공개 정보로만 반영된다
...
(`types.ts`/`deck.ts`/`handEvaluator.ts`/`equity.ts`는 사실상 동일본, `augmentEngine.ts`/`botAI.ts`는
서버 쪽에 멀티플레이 전용 로직이 더 있어 내용이 다릅니다). 룰을 고치면 양쪽을 함께 수정해야 합니다.
`botAI.ts`의 판단 로직과 `BOT_EQUITY_CONFIG` 임계값은 양쪽이 같은 값이어야 봇이 싱글/멀티에서
같게 행동합니다 — 임계값을 조정할 땐 두 파일을 반드시 함께 고쳐야 합니다.
...
│   │   └── useLiveEquity.ts     # 실시간 승률 훅 (내 카드만 사용, 상대 수는 공개 상태에서 집계)
│   └── components/            # 화면 & 위젯
│       ├── Card.tsx, AugmentCard.tsx, BettingPanel.tsx, TopBar.tsx, LogPanel.tsx, WinBanner.tsx
│       ├── EquityBadge.tsx              # 족보 캡슐 옆 `승률 63.2%` 표시
...
npm test                  # 서버: 엔진 유닛 테스트 (handEvaluator/augmentEngine/equity/botAI/showdownFlow)
npm run smoke              # 서버: 클라이언트 2명 자동 플레이 통합 테스트 (전체 게임 루프 검증)
npm run selfplay           # 서버: 봇끼리 자동 대전(기본 2000판) 후 판단 지표 리포트 출력
```

봇 임계값을 조정할 땐 `selfplay`로 조정 전후를 비교한다. 판수·인원·임계값을 인자로 바꿀 수 있다:

```bash
cd server
npm run selfplay -- --hands=500 --players=4
npm run selfplay -- --personaOffset.aggressive=0.02 --raiseFacingBet=0.72
npm run selfplay -- --iterations=300          # 롤아웃을 줄여 4배 빠르게 (설정 비교용)
```

리포트에서 볼 지표는 **손해 보는 콜 비율**과 **레이즈 비율**이다. 칩 수지(BB/100)는 노이즈
바닥이 ±50~75라 설정 비교에 쓰면 안 된다 (근거는 위 "셀프플레이 기반 봇 튜닝" 참고).
